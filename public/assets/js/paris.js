/**
 * Created by HOANGNGHIA on 1/1/2016.
 */
var isMobile = {
    Android: function() {
...
jQuery(document).ready(function($){
    ...
    if ($('#myWords').length){
        ...
        $("#myWords").wordsrotator({
            ...
            words: ['AWESOME', 'STUNNING', 'BEAUTIFUL']  //Array of words, it may contain HTML values
        });
    }
    ...
    if (isMobile.any() && $('#home-fsslider').length ||  true ){
        $('body > #load').remove();
    } else {
    ...
    if ($('#masterslider-showcase').length){
        ...
        slider.control('arrows' ,{insertTo:'#masterslider-showcase'});
    }
...
$(window).bind('load', function() {
    ...
    parallaxInit();
    $.waypoints("refresh");
    var t=setTimeout(function(){$.waypoints("refresh");},3); // 3000 original
...
jQuery(document).ready(function(){
    ...
    $('.contact-form #submit').click(function () {
        ...
        var servicetype = $('select[name=services]');
        var verifbox = $('input[name=verif_box]');
        ...
        else servicetype.removeClass('hightlight');

        if (verifbox.val()=='') {
            verifbox.addClass('hightlight');
            return false;
        } else verifbox.removeClass('hightlight');


        //organize the data properly
        var data = 'name=' + name.val() + '&email=' + email.val() + '&subject=' +
            subject.val() + '&comment='  + encodeURIComponent(comment.val()) + '&servicesval=' + servicetype.val()+'&verifybox='+ verifbox.val();
        ...
        $.ajax({
            ...
            success: function (html) {
                ...
                {
                    if(html=="wrong verification code.")
